...
public class AssignmentController {
    ...
    public ResponseEntity<ApiResponse> assign(@Valid @RequestBody AssignRequest request) {
        return ResponseEntity.status(201).body(new ApiResponse(CREATED, new AssignResponse(assignmentService.assign(request))));
    }
    ...
    public ResponseEntity<ApiResponse> findById(@PathVariable UUID id) {
        return ResponseEntity.status(200)
                .body(new ApiResponse(OK, new AssignResponse(assignmentService.findById(id))));
    }
