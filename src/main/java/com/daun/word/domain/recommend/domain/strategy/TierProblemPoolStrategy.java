...
public class TierProblemPoolStrategy implements ProblemPoolStrategy {
    ...
    public List<Problem> recommend(ProblemRepository problemRepository, RecommendSearchQuery query) {
        ...
        List<Problem> response = problemRepository.findAllByTierBetweenOrderByAcceptedUserCountDesc(query.getMinTier(), query.getMaxTier());
        if(response.isEmpty()) {
            throw new IllegalStateException("문제풀을 생성할 수 없습니다");
        }
        return response;
    }
}
